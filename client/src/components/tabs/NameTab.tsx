...
const NameTab = (props: { data: NameTabData; onChange: (change: NameTabData) => void; messages: Message[] }) => {
  const handleNameChange = (event: React.ChangeEvent<HTMLInputElement>) => props.onChange({ ...props.data, displayName: event.target.value });
  ...
  return (
    <div className='name-tab'>
      ...
      <LabelInput label='Description' htmlFor='description' message={MessageUtil.findMessage(props.messages, 'description')}>
        <Editor className='input' defaultValue={props.data.description} defaultLanguage='form' height='37px' defaultPath='root.form' options={MINIMAL_STYLE} theme='axon-input' onChange={handleDescriptionChange} />
      </LabelInput>
      <LabelInput label='Means / Documents' htmlFor='documents'>
        <EditTable data={props.data.documents} onChange={handleDocChange} />
      </LabelInput>
