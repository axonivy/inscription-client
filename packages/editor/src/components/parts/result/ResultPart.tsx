...
const ResultPart = ({ hideParamDesc }: { hideParamDesc?: boolean }) => {
  const { config, updater } = useResultData();
  ...
  return (
    <>
      <CollapsiblePart collapsibleLabel='Result parameters'>
        <ParameterTable data={config.result.params} onChange={change => updater('params', change)} hideDesc={hideParamDesc} />
      </CollapsiblePart>
      <MappingTree data={config.result.map} mappingInfo={mappingInfo} onChange={change => updater('map', change)} location='result.code' />
      <Fieldset label='Code'>
        <CodeEditor code={config.result.code} onChange={change => updater('code', change)} location='result.code' />
      </Fieldset>
    </>
  );
};
