...
const MailHeaderPart = () => {
  const { config, updater } = useMailHeaderData();
  ...
  return (
    <>
      <Fieldset label='Subject' {...subjectFieldset.labelProps}>
        <Input value={config.headers.subject} onChange={change => updater('subject', change)} {...subjectFieldset.inputProps} />
      </Fieldset>
      <Fieldset label='From' {...fromFieldset.labelProps}>
        <Input value={config.headers.from} onChange={change => updater('from', change)} {...fromFieldset.inputProps} />
      </Fieldset>
      <Fieldset label='Reply to' {...replyToFieldset.labelProps}>
        <Input value={config.headers.replyTo} onChange={change => updater('replyTo', change)} {...replyToFieldset.inputProps} />
      </Fieldset>
      <Fieldset label='To' {...toFieldset.labelProps}>
        <Input value={config.headers.to} onChange={change => updater('to', change)} {...toFieldset.inputProps} />
      </Fieldset>
      <Fieldset label='CC' {...ccFieldset.labelProps}>
        <Input value={config.headers.cc} onChange={change => updater('cc', change)} {...ccFieldset.inputProps} />
      </Fieldset>
      <Fieldset label='BCC' {...bccFieldset.labelProps}>
        <Input value={config.headers.bcc} onChange={change => updater('bcc', change)} {...bccFieldset.inputProps} />
      </Fieldset>
    </>
  );
};
